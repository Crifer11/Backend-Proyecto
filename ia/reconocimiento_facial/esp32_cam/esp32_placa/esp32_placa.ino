...
const char* ssid = "NOMBRE D ETU INTERNET";       
const char* password = "TU CONTRASEÑA DE INTERNET";  

// ==== CONFIGURA EL SERVIDOR FLASK ====
String serverName = "http://192.168.1.71:5000/upload_placa";  
...
void setup() {
  ...
  Serial.println("Frames iniciales descartados.");

  Serial.println("Escribe '1' en la consola para capturar una foto de placa.");
}

void capturarYEnviarFoto() {
  Serial.println("Capturando foto...");

  // ==== DESCARTAR UN FRAME EXTRA ====
  ...
  Serial.printf("Foto capturada! Tamano: %d bytes\n", fb->len);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      Serial.printf("Error al enviar la imagen. Codigo: %d\n", httpResponseCode);
    }
